# Defined in - @ line 1
function glg --description 'alias glg=git log --topo-order --all --graph --pretty=format:"$_git_log_oneline_format"'
	...
	set -l _git_log_oneline_format '%C(green)%h%C(reset) %s%C(red)%d%C(reset)'
